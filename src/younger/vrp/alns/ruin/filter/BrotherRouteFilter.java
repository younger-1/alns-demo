...
public class BrotherRouteFilter extends ALNSAbstractRuin implements IALNSRuin {

    private int maxCustomerNum;
    private int vehicleCapacity;
    private double travelTime;

    private BrotherRouteFilter() {
        random = new Random();
    ...
    public ALNSSolution ruin(ALNSSolution s, int nodes) throws Exception {
        ...
        Route centerRoute = Arrays.stream(routes).mapToObj(x -> s.routes.get(x)).filter(r -> r.getSize() > 2)
                .reduce((a, b) -> getFreeSpace(a) > getFreeSpace(b) ? a : b).get();
